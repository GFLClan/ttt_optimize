...
function SCORE:AddEvent(entry, t_override)
   entry.t = t_override or _CurTime()
   _table_insert(self.Events, entry)
end
...
local function CopyDmg(dmg)
   ...
   if wep then
      ...
      if _IsValid(infl) and infl.ScoreName then
         n = infl.ScoreName
      end
   end
...
function SCORE:HandleKill(victim, attacker, dmginfo)
   if not (_IsValid(victim) and victim:IsPlayer()) then return end
   ...
   if _IsValid(attacker) and attacker:IsPlayer() then
      ...
      if dmginfo:IsExplosionDamage() and attacker:GetTraitor() and victim:GetTraitor() then
         ...
         if _IsValid(infl) and infl:GetClass() == "ttt_c4" then
            e.att = _table_Copy(e.vic)
         end
      end
   end
...
function SCORE:HandleSelection()
   ...
   for k, ply in _ipairs(_player_GetAll()) do
      if ply:GetTraitor() then
         _table_insert(traitors, ply:SteamID())
      elseif ply:GetDetective() then
         _table_insert(detectives, ply:SteamID())
      end
   end
...
function SCORE:HandleC4Explosion(planter, arm_time, exp_time)
   local nick = "Someone"
   if _IsValid(planter) and planter:IsPlayer() then
      nick = planter:Nick()
   end
...
function SCORE:HandleC4Disarm(disarmer, owner, success)
   ...
   if not _IsValid(disarmer) then return end
...
function SCORE:ApplyEventLogScores(wintype)
   ...
   for i = 1, #events do
      ...
      if e.id == EVENT_KILL then
         local victim = _player_GetBySteamID(e.vic.sid)
         if _IsValid(victim) and victim:ShouldScore() then
            victim:AddDeaths(1)
         end
      end
   end
...
function SCORE:StreamToClients()
   local events = _util_TableToJSON(self.Events)
   if events == nil then
      _ErrorNoHalt("Round report event encoding failed!\n")
      return
   end

   events = _util_Compress(events)
   if events == "" then
      _ErrorNoHalt("Round report event compression failed!\n")
      return
   end
   ...
   if len <= MaxStreamLength then
      _net_Start("TTT_ReportStream")
         _net_WriteUInt(len, 16)
         _net_WriteData(events, len)
      _net_Broadcast()
   else
      local curpos = 0

      repeat
         _net_Start("TTT_ReportStream_Part")
            _net_WriteData(_string_sub(events, curpos + 1, curpos + MaxStreamLength + 1), MaxStreamLength)
         _net_Broadcast()
      ...
      _net_Start("TTT_ReportStream")
         _net_WriteUInt(len, 16)
         _net_WriteData(_string_sub(events, curpos + 1, len), len - curpos)
      _net_Broadcast()
   end
end
