...
_AccessorFunc( PANEL, "m_iMin",  "Min" )
_AccessorFunc( PANEL, "m_iMax",  "Max" )
_AccessorFunc( PANEL, "m_iValue",    "Value" )
_AccessorFunc( PANEL, "m_Color",     "Color" )

function PANEL:Init()
   self.Label = _vgui_Create( "DLabel", self )
   self.Label:SetFont( "DefaultSmall" )
   self.Label:SetColor( _Color( 0, 0, 0 ) )
   
   self:SetMin( 0 )
   self:SetMax( 1000 )
   self:SetValue( 253 )
   self:SetColor( _Color( 50, 205, 255, 255 ) )
end
...
function PANEL:UpdateText()
   ...
   if ( self.m_bLabelAsPercentage ) then
      self.Label:SetText( _Format( "%.2f%%", fDelta * 100 ) )
      return
   end
   
   if ( self.m_iMin == 0 ) then
      
      self.Label:SetText( _Format( "%i / %i", self.m_iValue, self.m_iMax ) )
...
function PANEL:Paint()
   ...
   _surface_SetDrawColor( 0, 0, 0, 170 )
   _surface_DrawRect( 0, 0, Width, self:GetTall() )
   
   _surface_SetDrawColor( self.m_Color.r, self.m_Color.g, self.m_Color.b, self.m_Color.a * 0.5 )
   _surface_DrawRect( 2, 2, Width - 4, self:GetTall() - 4 )
   _surface_SetDrawColor( self.m_Color.r, self.m_Color.g, self.m_Color.b, self.m_Color.a )
   _surface_DrawRect( 2, 2, Width * fDelta - 4, self:GetTall() - 4 )

end

_vgui_Register( "TTTProgressBar", PANEL, "DPanel" )
