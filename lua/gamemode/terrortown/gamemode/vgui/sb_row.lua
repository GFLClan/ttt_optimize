...
local _DermaMenu = (CLIENT and DermaMenu or nil)
...
function PANEL:Init()
   ...
   if KARMA.IsEnabled() then
      self:AddColumn( GetTranslation("sb_karma"), function(ply) return _math_Round(ply:GetBaseKarma()) end )
   end

   -- Let hooks add their custom columns
   _hook_Call("TTTScoreboardColumns", nil, self)

   for _, c in _ipairs(self.cols) do
      c:SetMouseInputEnabled(false)
   end

   self.tag = _vgui_Create("DLabel", self)
   self.tag:SetText("")
   self.tag:SetMouseInputEnabled(false)

   self.sresult = _vgui_Create("DImage", self)
   self.sresult:SetSize(16,16)
   self.sresult:SetMouseInputEnabled(false)

   self.avatar = _vgui_Create( "AvatarImage", self )
   self.avatar:SetSize(SB_ROW_HEIGHT, SB_ROW_HEIGHT)
   self.avatar:SetMouseInputEnabled(false)

   self.nick = _vgui_Create("DLabel", self)
   self.nick:SetMouseInputEnabled(false)

   self.voice = _vgui_Create("DImageButton", self)
...
local namecolor = {
   ...
   dev = _Color(100, 240, 105, 255)
}

local rolecolor = {
   default = _Color(0, 0, 0, 0),
   traitor = _Color(255, 0, 0, 30),
   detective = _Color(0, 0, 255, 30)
}

function GM:TTTScoreboardColorForPlayer(ply)
   if not _IsValid(ply) then return namecolor.default end

   if ply:SteamID() == "STEAM_0:0:1963640" then
      return namecolor.dev
   elseif ply:IsAdmin() and _GetGlobalBool("ttt_highlight_admins", true) then
...
function GM:TTTScoreboardRowColorForPlayer(ply)
   if not _IsValid(ply) then return rolecolor.default end
...
local function ColorForPlayer(ply)
   if _IsValid(ply) then
      local c = _hook_Call("TTTScoreboardColorForPlayer", GAMEMODE, ply)

      -- verify that we got a proper color
      if c and _istable(c) and c.r and c.b and c.g and c.a then
         return c
      else
         _ErrorNoHalt("TTTScoreboardColorForPlayer hook returned something that isn't a color!\n")
      end
   end
   return namecolor.default
end
...
--   end
   ...
   local c = _hook_Call("TTTScoreboardRowColorForPlayer", GAMEMODE, ply)

   _surface_SetDrawColor(c)
   _surface_DrawRect(0, 0, width, SB_ROW_HEIGHT)


   if ply == _LocalPlayer() then
      _surface_SetDrawColor( 200, 200, 200, _math_Clamp(_math_sin(_RealTime() * 2) * 50, 0, 100))
      _surface_DrawRect(0, 0, width, SB_ROW_HEIGHT )
   end
...
function PANEL:SetPlayer(ply)
   ...
   if not self.info then
      ...
      if g == GROUP_TERROR and ply != _LocalPlayer() then
         self.info = _vgui_Create("TTTScorePlayerInfoTags", self)
         self.info:SetPlayer(ply)

         self:InvalidateLayout()
      elseif g == GROUP_FOUND or g == GROUP_NOTFOUND then
         self.info = _vgui_Create("TTTScorePlayerInfoSearch", self)
         self.info:SetPlayer(ply)
         self:InvalidateLayout()
      end
   else
   ...
   self.voice.DoClick = function()
                           if _IsValid(ply) and ply != _LocalPlayer() then
...
function PANEL:UpdatePlayerData()
   ...
   if ply.search_result and (_LocalPlayer():IsDetective() or (not ply.search_result.show)) then
      self.sresult:SetImageColor(_Color(200, 200, 255))
   end
...
function PANEL:ApplySchemeSettings()
   ...
   self.sresult:SetImageColor(_Color(170, 170, 170, 150))
end
...
function PANEL:SetOpen(o)
   if self.open then
      _surface_PlaySound("ui/buttonclickrelease.wav")
   else
...
function PANEL:DoRightClick()
   local menu = _DermaMenu()
   menu.Player = self:GetPlayer()

   local close = _hook_Call( "TTTScoreboardMenu", nil, menu )
   if close then menu:Remove() return end

   menu:Open()
end

_vgui_Register( "TTTScorePlayerRow", PANEL, "DButton" )
