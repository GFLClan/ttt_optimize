...
local function PlayRadioSound(snd)
   local r = _LocalPlayer().radio
   if _IsValid(r) then
      _RunConsoleCommand("ttt_radio_play", _tostring(r:EntIndex()), snd)
   end
end
...
local function CreateSoundBoard(parent)
   local b = _vgui_Create("DPanel", parent)
   ...
   local x, y = 0, 0
   for ri, row in _ipairs(smatrix) do
      local rj = ri - 1 -- easier for computing x,y
      for rk, snd in _ipairs(row) do
         local rl = rk - 1
         y = (rj * m) + (rj * bh)
         x = (rl * m) + (rl * bw)

         local but = _vgui_Create("DButton", b)
         but:SetPos(x, y)
         but:SetSize(bw, bh)
