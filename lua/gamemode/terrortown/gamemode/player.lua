...
local _IsTableOfEntitiesValid = IsTableOfEntitiesValid
local _Vector = Vector
local _util_IsInWorld = (SERVER and util.IsInWorld or nil)
local _pairs = pairs
local _util_TraceLine = util.TraceLine
local _IsValid = IsValid
local _util_PointContents = util.PointContents
...
_CreateConVar("ttt_bots_are_spectators", "0", FCVAR_ARCHIVE)
_CreateConVar("ttt_dyingshot", "0")

_CreateConVar("ttt_killer_dna_range", "550")
_CreateConVar("ttt_killer_dna_basetime", "100")
...
function GM:PlayerInitialSpawn( ply )
   ...
   -- Handle spec bots
   if ply:IsBot() and _GetConVar("ttt_bots_are_spectators"):GetBool() then
...
function GM:NetworkIDValidated( name, steamid )
   ...
   for _, p in _ipairs(_player_GetAll()) do
      if _IsValid(p) and p:SteamID() == steamid and p.delay_karma_recall then
...
function GM:PlayerSpawn(ply)
   ...
   _net_Start("TTT_PlayerSpawned")
      _net_WriteBit(ply:IsSpec())
   _net_Send(ply)
   ...
   _hook_Call("PlayerSetModel", GAMEMODE, ply)
   _hook_Call("TTTPlayerSetColor", GAMEMODE, ply)
...
function GM:IsSpawnpointSuitable(ply, spwn, force, rigged)
   if not _IsValid(ply) or not ply:IsTerror() then return true end
   if not rigged and (not _IsValid(spwn) or not spwn:IsInWorld()) then return false end

   -- spwn is normally an ent, but we sometimes use a vector for jury rigged
   -- positions
   local pos = rigged and spwn or spwn:GetPos()

   if not _util_IsInWorld(pos) then return false end

   local blocking = ents.FindInBox(pos + _Vector( -16, -16, 0 ), pos + _Vector( 16, 16, 64 ))

   for k, p in _ipairs(blocking) do
      if _IsValid(p) and p:IsPlayer() and p:IsTerror() and p:Alive() then
         if force then
            p:Kill()
         else
            return false
...
function GetSpawnEnts(shuffled, force_all)
   local tbl = {}
   for k, classname in _ipairs(SpawnTypes) do
      for _, e in _ipairs(ents.FindByClass(classname)) do
         if _IsValid(e) and (not e.BeingRemoved) then
            _table_insert(tbl, e)
         end
      end
   end
   ...
   if force_all or #tbl == 0 then
      for _, e in _ipairs(ents.FindByClass("info_player_start")) do
         if _IsValid(e) and (not e.BeingRemoved) then
            _table_insert(tbl, e)
         end
      end
...
local function PointsAroundSpawn(spwn)
   if not _IsValid(spwn) then return {} end
...
function GM:PlayerSelectSpawn(ply)
   if (not self.SpawnPoints) or (table.IsEmpty(self.SpawnPoints)) or (not _IsTableOfEntitiesValid(self.SpawnPoints)) then
   ...
   for k, spwn in pairs(self.SpawnPoints) do
      ...
      for _, rig in pairs(rigged) do
         if self:IsSpawnpointSuitable(ply, rig, false, true) then
            ...
            if _IsValid(rig_spwn) then
               rig_spwn:SetPos(rig)
               rig_spwn:Spawn()

               _ErrorNoHalt("TTT WARNING: Map has too few spawn points, using a rigged spawn for ".. _tostring(ply) .. "\n")
...
function GM:TTTPlayerSetColor(ply)
   ...
   ply:SetPlayerColor( _Vector( clr.r/255.0, clr.g/255.0, clr.b/255.0 ) )
end
...
function GM:PlayerSwitchFlashlight(ply, on)
   if not _IsValid(ply) then return false end
...
function GM:PlayerSpray(ply)
   if not _IsValid(ply) or not ply:IsTerror() then
      return true -- block
   end
end
...
function GM:KeyPress(ply, key)
   ...
   if ply:IsSpec() and not ply:GetRagdollSpec() then
      ...
      if key == IN_ATTACK then
         ...
         local target = _table_Random(alive)
         if _IsValid(target) then
      ...
      elseif key == IN_DUCK then
         ...
         if _IsValid(target) and target:IsPlayer() and ply:GetObserverMode() != OBS_MODE_ROAMING then -- Only set the spectator's position to the player they are spectating if they are in chase or eye mode. They can use the reload key if they want to return to the person they're spectating
            pos = target:EyePos()
      ...
      elseif key == IN_RELOAD then
         ...
         if not _IsValid(tgt) or not tgt:IsPlayer() then return end
...
function GM:KeyRelease(ply, key)
   if key == IN_USE and _IsValid(ply) and ply:IsTerror() then
      ...
      if tr.Hit and _IsValid(tr.Entity) then
         if tr.Entity.CanUseKey and tr.Entity.UseOverride then
            local phys = tr.Entity:GetPhysicsObject()
            if _IsValid(phys) and not phys:HasGameFlag(FVPHYSICS_PLAYER_HELD) then
...
local function SpecUseKey(ply, cmd, arg)
   if _IsValid(ply) and ply:IsSpec() then
      -- longer range than normal use
      local tr = _util_QuickTrace(ply:GetShootPos(), ply:GetAimVector() * 128, ply)
      if tr.Hit and _IsValid(tr.Entity) then
...
_concommand_Add("ttt_spec_use", SpecUseKey)
...
local function CreateDeathEffect(ent, marked)
   local pos = ent:GetPos() + _Vector(0, 0, 20)

   local jit = 35.0

   local jitter = _Vector(_math_Rand(-jit, jit), _math_Rand(-jit, jit), 0)
...
local deathsounds = {
   _Sound("player/death1.wav"),
   _Sound("player/death2.wav"),
   _Sound("player/death3.wav"),
   _Sound("player/death4.wav"),
   _Sound("player/death5.wav"),
   _Sound("player/death6.wav"),
   _Sound("vo/npc/male01/pain07.wav"),
   _Sound("vo/npc/male01/pain08.wav"),
   _Sound("vo/npc/male01/pain09.wav"),
   _Sound("vo/npc/male01/pain04.wav"),
   _Sound("vo/npc/Barney/ba_pain06.wav"),
   _Sound("vo/npc/Barney/ba_pain07.wav"),
   _Sound("vo/npc/Barney/ba_pain09.wav"),
   _Sound("vo/npc/Barney/ba_ohshit03.wav"), --heh
   _Sound("vo/npc/Barney/ba_no01.wav"),
   _Sound("vo/npc/male01/no02.wav"),
   _Sound("hostage/hpain/hpain1.wav"),
   _Sound("hostage/hpain/hpain2.wav"),
   _Sound("hostage/hpain/hpain3.wav"),
   _Sound("hostage/hpain/hpain4.wav"),
   _Sound("hostage/hpain/hpain5.wav"),
   _Sound("hostage/hpain/hpain6.wav")
};
...
local function PlayDeathSound(victim)
   ...
   sound.Play(_table_Random(deathsounds), victim:GetShootPos(), 90, 100)
end
...
local function CheckCreditAward(victim, attacker)
   ...
   -- DETECTIVE AWARD
   if _IsValid(attacker) and attacker:IsPlayer() and attacker:IsActiveDetective() and victim:IsTraitor() then
      local amt = _GetConVarNumber("ttt_det_credits_traitordead") or 1
   ...
   -- TRAITOR AWARD
   if (not victim:IsTraitor()) and (not GAMEMODE.AwardedCredits or _GetConVar("ttt_credits_award_repeat"):GetBool()) then
      local inno_alive = 0
      local inno_dead = 0
      local inno_total = 0
      ...
      inno_dead = inno_dead + 1
      inno_alive = _math_max(inno_alive - 1, 0)
      ...
      if pct >= _GetConVarNumber("ttt_credits_award_pct") then
         -- Traitors have killed sufficient people to get an award
         local amt = _GetConVarNumber("ttt_credits_award_size")
...
function GM:DoPlayerDeath(ply, attacker, dmginfo)
   ...
   if _GetConVar("ttt_dyingshot"):GetBool() then
      local wep = ply:GetActiveWeapon()
      if _IsValid(wep) and wep.DyingShot and not ply.was_headshot and dmginfo:IsBulletDamage() then
   ...
   -- Drop all weapons
   for k, wep in _ipairs(ply:GetWeapons()) do
      WEPS.DropNotifiedWeapon(ply, wep, true) -- with ammo in them
      wep:DampenDrop()
   end

   if _IsValid(ply.hat) then
      ply.hat:Drop()
   end
   ...
   if GetRoundState() == ROUND_ACTIVE then
      ...
      if _IsValid(attacker) and attacker:IsPlayer() then
         ...
         DamageLog(_Format("KILL:\t %s [%s] killed %s [%s]", attacker:Nick(), attacker:GetRoleString(), ply:Nick(), ply:GetRoleString()))
      else
         DamageLog(_Format("KILL:\t <something/world> killed %s [%s]", ply:Nick(), ply:GetRoleString()))
      end
   ...
   if not (ply.was_headshot or
           ...
           (_IsValid(killwep) and killwep.IsSilent)) then
      PlayDeathSound(ply)
   end
   ...
   if _IsValid(attacker) and attacker:IsPlayer() then
      ...
      if attacker:IsActiveTraitor() and ply:GetDetective() then
         reward = _math_ceil(_GetConVarNumber("ttt_credits_detectivekill"))
      elseif attacker:IsActiveDetective() and ply:GetTraitor() then
         reward = _math_ceil(_GetConVarNumber("ttt_det_credits_traitorkill"))
      end
...
function GM:PlayerDeath(victim, infl, attacker)
   ...
   _net_Start("TTT_PlayerDied") _net_Send(victim)

   if HasteMode() and GetRoundState() == ROUND_ACTIVE then
      IncRoundEnd(_GetConVar("ttt_haste_minutes_per_death"):GetFloat() * 60)
   end
end
...
function GM:SpectatorThink(ply)
   ...
   if ply:GetRagdollSpec() then
      ...
      local elapsed = _CurTime() - ply.spec_ragdoll_start
      ...
      if (m == OBS_MODE_CHASE and clicked) or elapsed > to_roam then
         ...
         if spec_spawns and #spec_spawns > 0 then
            local spawn = _table_Random(spec_spawns)
      ...
      if not _IsValid(ply.server_ragdoll) then ply:SetRagdollSpec(false) end
...
function GM:PlayerTraceAttack(ply, dmginfo, dir, trace)
   if _IsValid(ply.hat) and trace.HitGroup == HITGROUP_HEAD then
      ply.hat:Drop(dir)
   end
...
local fallsounds = {
   _Sound("player/damage1.wav"),
   _Sound("player/damage2.wav"),
   _Sound("player/damage3.wav")
};

function GM:OnPlayerHitGround(ply, in_water, on_floater, speed)
   if in_water or speed < 450 or not _IsValid(ply) then return end

   -- Everything over a threshold hurts you, rising exponentially with speed
   local damage = _math_pow(0.05 * (speed - 420), 1.75)
   ...
   if _IsValid(ground) and ground:IsPlayer() then
      if _math_floor(damage) > 0 then
         local att = ply
         ...
         if push then
            ...
            if _math_max(push.t or 0, push.hurt or 0) > _CurTime() - 4 then
               att = push.att
            end
         end

         local dmg = _DamageInfo()
         ...
         dmg:SetDamageForce(_Vector(0,0,-1))
   ...
   if _math_floor(damage) > 0 then
      ...
      if damage > 5 then
         sound.Play(_table_Random(fallsounds), ply:GetShootPos(), 55 + _math_Clamp(damage, 0, 50), 100)
      end
   end
end

local ttt_postdm = _CreateConVar("ttt_postround_dm", "0", FCVAR_NOTIFY)
...
function GM:EntityTakeDamage(ent, dmginfo)
   ...
   if not GAMEMODE:AllowPVP() then
      ...
      if (ent:IsExplosive() or (ent:IsPlayer() and _IsValid(att) and att:IsPlayer())) then
   ...
   elseif ent:IsExplosive() then
      ...
      if _IsValid(att) and att:IsPlayer() and
         dmginfo:IsDamageType(DMG_CRUSH) and
         _IsValid(ent:GetPhysicsAttacker()) then
...
function GM:PlayerTakeDamage(ent, infl, att, amount, dmginfo)
   ...
   if infl or att then
      ...
      if _IsValid(infl) then
         hurter = infl
      elseif _IsValid(att) then
         hurter = att
      end

      -- have a damage owner?
      if hurter and _IsValid(hurter:GetDamageOwner()) then
      ...
      elseif hurter and ent == hurter:GetPhysicsAttacker() and dmginfo:IsDamageType(DMG_BLAST) then
         owner = ent
      elseif hurter and hurter:IsVehicle() and _IsValid(hurter:GetDriver()) then
      ...
      if owner_time or (not _IsValid(att)) or (not att:IsPlayer()) then
         local push = ent.was_pushed

         if push and _IsValid(push.att) and push.t then
            -- push must be within the last 5 seconds, and must be done
            -- after the trap was enabled (if any)
            owner_time = owner_time or 0
            local t = _math_max(push.t or 0, push.hurt or 0)
            if t > owner_time and t > _CurTime() - 4 then
               owner = push.att

               -- pushed by a trap?
               if _IsValid(push.infl) then
      ...
      if _IsValid(owner) and dmginfo:IsDamageType(DMG_CRUSH) then
         ...
         if push.att != owner then
            owner_time = owner_time or _CurTime()
      ...
      att = _IsValid(owner) and owner or att
      dmginfo:SetAttacker(att)
   end

   -- scale phys damage caused by props
   if dmginfo:IsDamageType(DMG_CRUSH) and _IsValid(att) then
      ...
      if not dmginfo:IsDamageType(DMG_PHYSGUN) then
         ...
         if _IsValid(infl) and _IsValid(infl:GetOwner()) and infl:GetOwner():IsPlayer() then
            dmginfo:ScaleDamage(0)
            dmginfo:SetDamage(0)
         end
      end
   ...
   if ent.ignite_info and dmginfo:IsDamageType(DMG_DIRECT) then
      ...
      if (not _IsValid(datt)) or (not datt:IsPlayer()) then
         local ignite = ent.ignite_info
         if _IsValid(ignite.att) and _IsValid(ignite.infl)then
   ...
   if ent.was_pushed and ent == att and dmginfo:GetDamageType() == DMG_GENERIC and util.BitSet(_util_PointContents(dmginfo:GetDamagePosition()), CONTENTS_WATER) then
      local t = _math_max(ent.was_pushed.t or 0, ent.was_pushed.hurt or 0)
      if t > _CurTime() - 3 then
         dmginfo:SetAttacker(ent.was_pushed.att)
         ent.was_pushed.hurt = _CurTime()
      end
   end
   ...
   if ent != att and _IsValid(att) and att:IsPlayer() and GetRoundState() == ROUND_ACTIVE and _math_floor(dmginfo:GetDamage()) > 0 then
      ...
      DamageLog(_Format("DMG: \t %s [%s] damaged %s [%s] for %d dmg", att:Nick(), att:GetRoleString(), ent:Nick(), ent:GetRoleString(), _math_Round(dmginfo:GetDamage())))
   end
   ...
   for i= 1, #plys do
      ...
      if tm == TEAM_TERROR and ply:Alive() then
         ...
         if ply:WaterLevel() == 3 then
            ...
            if ply.drowning then
               if ply.drowning < _CurTime() then
                  local dmginfo = _DamageInfo()
                  dmginfo:SetDamage(15)
                  dmginfo:SetDamageType(DMG_DROWN)
                  dmginfo:SetAttacker(game.GetWorld())
                  dmginfo:SetInflictor(game.GetWorld())
                  dmginfo:SetDamageForce(_Vector(0,0,1))

                  ply:TakeDamageInfo(dmginfo)

                  -- have started drowning properly
                  ply.drowning = _CurTime() + 1
               end
            else
               -- will start drowning soon
               ply.drowning = _CurTime() + 8
            end
         else
            ply.drowning = nil
         end
         ...
         if _IsValid(ply.scanner_weapon) and wep != ply.scanner_weapon then
      ...
      elseif tm == TEAM_SPEC then
         if ply.propspec then
            ...
            if _IsValid(ply:GetObserverTarget()) then
