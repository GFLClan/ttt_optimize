...
function ScoreEventLog(events, scores, traitors, detectives)
   for k, s in _pairs(scores) do
      scores[k] = ScoreInit()

      scores[k].was_traitor = _table_HasValue(traitors, k)
      scores[k].was_detective = _table_HasValue(detectives, k)
   end

   local tmp = nil
   for k, e in _pairs(events) do
      ScoreEvent(e, scores)
   end
...
function ScoreTeamBonus(scores, wintype)
   ...
   local bonus = {}
   bonus.traitors = (alive.traitors * 1) + _math_ceil(dead.innos * 0.5)
   bonus.innos = alive.innos * 1

   -- running down the clock must never be beneficial for traitors
   if wintype == WIN_TIMELIMIT then
      bonus.traitors = _math_floor(alive.innos * -0.5) + _math_ceil(dead.innos * 0.5)
   end
...
function GetWeaponClassNames()
   if not WeaponNames then
      ...
      for k,v in _pairs(scripted_ents.GetList()) do
