...
local function GetTextForRole(role)
   ...
   elseif role == ROLE_DETECTIVE then
      ...
      if #traitors > 1 then
         ...
         for k, ply in _ipairs(traitors) do
            if ply != _LocalPlayer() then
               traitorlist = traitorlist .. _string_rep(" ", 42) .. ply:Nick()  .. "\n"
            end
         end
...
local startshowtime = _CreateConVar("ttt_startpopup_duration", "17", FCVAR_ARCHIVE)
...
local function RoundStartPopup()
   ...
   if not _LocalPlayer() then return end

   local dframe = _vgui_Create( "Panel" )
   dframe:SetDrawOnTop( true )
   dframe:SetMouseInputEnabled(false)
   dframe:SetKeyboardInputEnabled(false)

   local color = _Color(0,0,0, 200)
   dframe.Paint = function(s)
                     _draw_RoundedBox(8, 0, 0, s:GetWide(), s:GetTall(), color)
                  end


   local text = GetTextForRole(_LocalPlayer():GetRole())

   local dtext = _vgui_Create( "DLabel", dframe )
   ...
   _timer_Simple(startshowtime:GetInt(), function() dframe:Remove() end)
end
_concommand_Add("ttt_cl_startpopup", RoundStartPopup)
...
local function IdlePopup()
   ...
   local bw, bh = 75, 25
   local cancel = _vgui_Create("DButton", dframe)
...
_concommand_Add("ttt_cl_idlepopup", IdlePopup)
