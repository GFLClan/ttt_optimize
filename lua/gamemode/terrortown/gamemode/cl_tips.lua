...
_CreateConVar("ttt_tips_enable", "1", FCVAR_ARCHIVE)
...
function PANEL:Paint()
   ...
   _surface_SetDrawColor(clr.r, clr.g, clr.b, clr.a)
...
local tips_bg = _Color(0, 0, 0, 200)

local tip_ids = {}
for i=1, 40 do
   _table_insert(tip_ids, i)
end
...
function PANEL:NextTip(auto)
   ...
   self.NextSwitch = _CurTime() + (auto and self.AutoDelay or self.ManualDelay)
end
...
function PANEL:PerformLayout()
   ...
   local off_left = 260
   local off_right = 250
   local room = _ScrW() - off_left - off_right
   local width = _math_min(room, self.IdealWidth)
   ...
   local height = _math_max(self.IdealHeight, self.tiptext:GetTall() + m*2)

   local x = off_left + ((room - width) / 2)
   local y = _ScrH() - off_bottom - height
...
function PANEL:ApplySchemeSettings()
   for k, but in _pairs(self.buttons) do
...
function PANEL:Paint()
   _draw_RoundedBox(8, 0, 0, self:GetWide(), self:GetTall(), self.BgColor)
end

function PANEL:Think()
   if self.NextSwitch < _CurTime() then
      self:NextTip(true)
   end
end

_vgui_Register("TTTTips", PANEL, "Panel")
...
function TIPS.Create()
   if _IsValid(tips_panel) then
      tips_panel:Remove()
      tips_panel = nil
   end

   tips_panel = _vgui_Create("TTTTips")

   -- workaround for layout oddities, give it a poke next tick
   _timer_Simple(0.1, TIPS.Next)
end

function TIPS.Show()
   if not _GetConVar("ttt_tips_enable"):GetBool() then return end
...
_concommand_Add("ttt_tips_hide", TIPS.Hide)
