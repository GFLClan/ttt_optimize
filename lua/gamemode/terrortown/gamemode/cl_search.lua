...
local function IconForInfoType(t, data)
   ...
   if _istable(mat) then
      mat = mat[data]
   elseif _isfunction(mat) then
      mat = mat(data)
   end
...
function PreprocSearch(raw)
   ...
   for t, d in _pairs(raw) do
      ...
      elseif t == "words" then
         if d != "" then
            ...
            local final = _string_match(d, "[\\.\\!\\?]$") != nil
      ...
      elseif t == "kills" then
         local num = _table_Count(d)
         if num == 1 then
            local vic = _Entity(d[1])
            local dc = d[1] == -1 -- disconnected
            if dc or (_IsValid(vic) and vic:IsPlayer()) then
         ...
         elseif num > 1 then
            ...
            local nicks = {}
            for k, idx in _pairs(d) do
               local vic = _Entity(idx)
               local dc = idx == -1
               if dc or (_IsValid(vic) and vic:IsPlayer()) then
                  _table_insert(nicks, (dc and "<Disconnected>" or vic:Nick()))
               end
            end

            local last = #nicks
            txt = txt .. _table_concat(nicks, "\n", 1, last)
            search[t].text = txt
         end
   ...
   _hook_Call("TTTBodySearchPopulate", nil, search, raw)
...
local function SearchInfoController(search, dactive, dtext)
   return function(s, pold, pnew)
             ...
             local data = search[t]
             if not data then
                _ErrorNoHalt("Search: data not found", t, data,"\n")
                return
             end
...
local function ShowSearchScreen(search_raw)
   ...
   -- contents wrapper
   local dcont = _vgui_Create("DPanel", dframe)
   dcont:SetPaintBackground(false)
   dcont:SetSize(rw, rh)
   dcont:SetPos(m, 25 + m)

   -- icon list
   local dlist = _vgui_Create("DPanelSelect", dcont)
   ...
   -- description area
   local dscroll = _vgui_Create("DHorizontalScroller", dlist)
   dscroll:StretchToParent(3,3,3,3)

   local ddesc = _vgui_Create("ColoredBox", dcont)
   ddesc:SetColor(_Color(50, 50, 50))
   ddesc:SetName(T("search_info"))
   ddesc:SetPos(descx, descy)
   ddesc:SetSize(descw, desch)

   local dactive = _vgui_Create("DImage", ddesc)
   dactive:SetImage("vgui/ttt/icon_id")
   dactive:SetPos(m, m)
   dactive:SetSize(64, 64)

   local dtext = _vgui_Create("ScrollLabel", ddesc)
   ...
   dcall.DoClick = function(s)
                      ...
                      _table_insert(client.called_corpses, search_raw.eidx)
                      s:SetDisabled(true)

                      _RunConsoleCommand("ttt_call_detective", search_raw.eidx)
                   end

   dcall:SetDisabled(client:IsSpec() or _table_HasValue(client.called_corpses or {}, search_raw.eidx))

   local dconfirm = _vgui_Create("DButton", dcont)
   ...
   for t, info in _SortedPairsByMemberValue(search, "p") do
      ...
      if t == "nick" then
         local avply = _IsValid(search_raw.owner) and search_raw.owner or nil

         ic = _vgui_Create("SimpleIconAvatar", dlist)
         ic:SetPlayer(avply)
      ...
      elseif info.text_icon then
         ic = _vgui_Create("SimpleIconLabelled", dlist)
...
local function StoreSearchResult(search)
   if search.owner then
      ...
      if (not ply.search_result) or ply.search_result.show then
         ...
         local rag = _Entity(search.eidx)
         if _IsValid(rag) then
...
local function ReceiveRagdollSearch()
   ...
   -- Basic info
   search.eidx = _net_ReadUInt(16)

   search.owner = _Entity(_net_ReadUInt(8))
   if not (_IsValid(search.owner) and search.owner:IsPlayer() and (not search.owner:IsTerror())) then
      search.owner = nil
   end

   search.nick = _net_ReadString()

   -- Equipment
   local eq = _net_ReadUInt(16)
   ...
   -- Traitor things
   search.role = _net_ReadUInt(2)
   search.c4 = _net_ReadInt(bitsRequired(C4_WIRE_COUNT) + 1)

   -- Kill info
   search.dmg = _net_ReadUInt(30)
   search.wep = _net_ReadString()
   search.head = _net_ReadBit() == 1
   search.dtime = _net_ReadInt(16)
   search.stime = _net_ReadInt(16)

   -- Players killed
   local num_kills = _net_ReadUInt(8)
   if num_kills > 0 then
      search.kills = {}
      for i=1,num_kills do
         _table_insert(search.kills, _net_ReadUInt(8))
      end
   else
      search.kills = nil
   end

   search.lastid = {idx=_net_ReadUInt(8)}

   -- should we show a menu for this result?
   search.finder = _net_ReadUInt(8)

   search.show = (_LocalPlayer():EntIndex() == search.finder)
   ...
   _hook_Call("TTTBodySearchEquipment", nil, search, eq)
...
_net_Receive("TTT_RagdollSearch", ReceiveRagdollSearch)
