...
local is_dmg = function(dmg_t, bit)
                  ...
                  return util.BitSet(_tonumber(dmg_t), bit)
               end
...
local function ExplodedSelf(events, scores, players, traitors)
   for k, e in pairs(events) do
      if e.id == EVENT_KILL and is_dmg(e.dmg.t, DMG_BLAST) and e.att.sid == e.vic.sid then
         return {title=T("aw_exp2_title"), text=T("aw_exp2_text"), nick=e.vic.ni, priority=_math_random(1, 4)}
      end
   end
...
local function FirstBlood(events, scores, players, traitors)
   for k, e in pairs(events) do
      if e.id == EVENT_KILL and e.att.sid != e.vic.sid and e.att.sid != -1 then
         ...
         award.priority = _math_random(-3, _math_Round(_table_Count(players) / 4))
...
local function AllKills(events, scores, players, traitors)
   ...
   for id, s in pairs(scores) do
      if s.innos > 0 then
         _table_insert(in_killers, id)
      elseif s.traitors > 0 then
         _table_insert(tr_killers, id)
      end
   end

   if #tr_killers == 1 then
      local id = tr_killers[1]
      if not _table_HasValue(traitors, id) then
         local killer = players[id]
         if not killer then return nil end

         return {nick=killer, title=T("aw_all1_title"), text=T("aw_all1_text"), priority=_math_random(0, _table_Count(players))}
      end
   end

   if #in_killers == 1 then
      local id = in_killers[1]
      if _table_HasValue(traitors, id) then
         local killer = players[id]
         if not killer then return nil end

         return {nick=killer, title=T("aw_all2_title"), text=T("aw_all2_text"), priority=_math_random(0, _table_Count(players))}
      end
   end
...
local function NumKills_Traitor(events, scores, players, traitors)
   ...
   for id, s in pairs(scores) do
      if _table_HasValue(traitors, id) then
         if s.innos > 0 then
            _table_insert(trs, id)
         end
      end
   end

   local choices = _table_Count(trs)
   if choices > 0 then
   ...
   for id, s in pairs(scores) do
      if not _table_HasValue(traitors, id) then
         if s.traitors > 0 then
            _table_insert(ins, id)
         end
      end
   end

   local choices = _table_Count(ins)
...
local function FallDeath(events, scores, players, traitors)
   for k, e in pairs(events) do
      if e.id == EVENT_KILL and is_dmg(e.dmg.t, DMG_FALL) then
         if e.att.ni != "" then
            return {title=T("aw_fal1_title"), nick=e.att.ni, text=T("aw_fal1_text"), priority=_math_random(7, 15)}
         else
            return {title=T("aw_fal2_title"), nick=e.vic.ni, text=T("aw_fal2_text"), priority=_math_random(1, 5)}
         end
      end
   end
...
local function FallKill(events, scores, players, traitors)
   for k, e in pairs(events) do
      if e.id == EVENT_KILL and is_dmg(e.dmg.t, DMG_CRUSH) and is_dmg(e.dmg.t, DMG_PHYSGUN) then
         if e.att.ni != "" then
            return {title=T("aw_fal3_title"), nick=e.att.ni, text=T("aw_fal3_text"), priority=_math_random(10, 15)}
         end
      end
   end
end
...
local function CrowbarUser(events, scores, players, traitors)
   ...
   local award = {nick=nick, priority=most.kills + _math_random(0, 4)}
   ...
   elseif kills >= 3 then
      ...
      award.priority = kills + _math_random(5, 10)
   else
      return nil
   end
...
local function PistolUser(events, scores, players, traitors)
   ...
   elseif kills >= 4 then
      ...
      award.priority = award.priority + _math_random(0, 5)
   else
      return nil
   end
...
local function ShotgunUser(events, scores, players, traitors)
   ...
   if kills > 1 and kills < 4 then
      ...
      award.priority = _math_Round(kills / 2)
   elseif kills >= 4 then
...
local function DeagleUser(events, scores, players, traitors)
   ...
   elseif kills >= 4 then
      ...
      award.priority = kills + _math_random(2, 6)
   else
      return nil
   end
...
local function KnifeUser(events, scores, players, traitors)
   ...
   if kills == 1 then
      if _table_HasValue(traitors, most.sid) then
...
local function TeamKiller(events, scores, players, traitors)
   local num_traitors = _table_Count(traitors)
   local num_inno = _table_Count(players) - num_traitors
   ...
   local was_traitor = _table_HasValue(traitors, tker)
   ...
   elseif pct >= 1.0 then
      ...
      award.priority = kills + _math_random(3, 6)
   ...
   elseif pct > 0.5 then
      ...
      award.priority = kills + _math_random(2, 7)
...
local function Burner(events, scores, players, traitors)
   ...
   elseif m_num >= 7 then
      ...
      award.priority = m_num + _math_random(0, 4)
   else
      return nil
   end
...
local function CreditFound(events, scores, players, traitors)
   ...
   if m_num > 2 then
      ...
      award.priority = m_num + _math_random(0, m_num)
   else
      return nil
   end
