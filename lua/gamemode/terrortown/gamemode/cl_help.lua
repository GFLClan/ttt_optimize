...
_CreateConVar("ttt_spectator_mode", "0", FCVAR_ARCHIVE)
_CreateConVar("ttt_mute_team_check", "0")

_CreateClientConVar("ttt_avoid_detective", "0", true, true)

HELPSCRN = {}

local dframe
function HELPSCRN:Show()
   if _IsValid(dframe) then return end
   local margin = 15

   dframe = _vgui_Create("DFrame")
   local w, h = 630, 470
   dframe:SetSize(w, h)
   dframe:Center()
   ...
   local dbut = _vgui_Create("DButton", dframe)
   local bw, bh = 50, 25
   dbut:SetSize(bw, bh)
   ...
   local dtabs = _vgui_Create("DPropertySheet", dframe)
   ...
   local tutparent = _vgui_Create("DPanel", dtabs)
   ...
   local dsettings = _vgui_Create("DPanelList", dtabs)
   ...
   local dgui = _vgui_Create("DForm", dsettings)
   ...
   local dplay = _vgui_Create("DForm", dsettings)
   ...
   mute:SetValue(_GetConVar("ttt_mute_team_check"):GetBool())
   mute:SetTooltip(GetTranslation("set_mute_tip"))

   dsettings:AddItem(dplay)

   --- Language area
   local dlanguage = _vgui_Create("DForm", dsettings)
   dlanguage:SetName(GetTranslation("set_title_lang"))

   local dlang = _vgui_Create("DComboBox", dlanguage)
   dlang:SetConVar("ttt_language")

   dlang:AddChoice("Server default", "auto")
   for _, lang in _pairs(LANG.GetLanguages()) do
   ...
   dlang.OnSelect = function(idx, val, data)
                       _RunConsoleCommand("ttt_language", data)
                    end
   ...
   _hook_Call("TTTSettingsTabs", GAMEMODE, dtabs)
...
local function MuteTeamCallback(cv, old, new)
   local num = _tonumber(new)
   if num and (num == 0 or num == 1) then
      _RunConsoleCommand("ttt_mute_team", num)
   end
end
...
function HELPSCRN:CreateTutorial(parent)
   ...
   local bar = _vgui_Create("TTTProgressBar", parent)
   bar:SetSize(200, bh)
   bar:MoveBelow(bg)
   ...
   bar:SetValue(1)
   bar:SetColor(_Color(0,200,0))

   -- fixing your panels...
   bar.UpdateText = function(s)
                       s.Label:SetText(_Format("%i / %i", s.m_iValue, s.m_iMax))
                    end

   bar:UpdateText()


   local bnext = _vgui_Create("DButton", parent)
   ...
   local bprev = _vgui_Create("DButton", parent)
   ...
   bnext.DoClick = function()
                      if tut.current < tutorial_pages then
                         ...
                         tut:SetImage(_Format(imgpath, tut.current))
