...
_CreateConVar("ttt_language", "auto", FCVAR_ARCHIVE)
...
function LANG.CreateLanguage(lang_name)
   ...
   lang_name = _string_lower(lang_name)
   ...
   if lang_name == LANG.DefaultLanguage then
      ...
      _setmetatable(LANG.Strings[lang_name],
                   {
                      __index = function(tbl, name)
                                   return _Format("[ERROR: Translation of %s not found]", name), false
                                end
                   })
   end
...
function LANG.AddToLanguage(lang_name, string_name, string_text)
   ...
   if not LANG.IsLanguage(lang_name) then
      _ErrorNoHalt(_Format("Failed to add '%s' to language '%s': language does not exist.\n", _tostring(string_name), _tostring(lang_name)))
   end
...
function LANG.GetRawTranslation(name)
   return _rawget(cached_active, name) or _rawget(cached_default, name)
end
...
function LANG.GetTranslationFromLanguage(name, lang_name)
   return _rawget(LANG.Strings[lang_name], name)
end
...
function LANG.GetLanguageTable(lang_name)
   ...
   local cpy = _table_Copy(LANG.Strings[lang_name])
   SetFallback(cpy)
...
function LANG.SetActiveLanguage(lang_name)
   ...
   if LANG.IsLanguage(lang_name) then
      ...
      if old_name != lang_name then
         _hook_Call("TTTLanguageChanged", GAMEMODE, old_name, lang_name)
      end
   else
      _MsgN(_Format("The language '%s' does not exist on this server. Falling back to English...", lang_name))
...
function LANG.Init()
   local lang_name = _GetConVarString("ttt_language")
...
_concommand_Add("ttt_reloadlang", ForceReload)

-- Get a copy of all available languages (keys in the Strings tbl)
function LANG.GetLanguages()
   local langs = {}
   for lang, strings in _pairs(LANG.Strings) do
      _table_insert(langs, lang)
   end
   return langs
end
...
local bgcolor = {
   [ROLE_TRAITOR]   = _Color(150, 0, 0, 200),
   [ROLE_DETECTIVE] = _Color(0, 0, 150, 200),
   [ROLE_INNOCENT]  = _Color(0, 50,  0, 200)
};
...
for style, msgs in _pairs(styledmessages) do
   for _, name in _ipairs(msgs) do
