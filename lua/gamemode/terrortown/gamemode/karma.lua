...
KARMA.cv = {}
KARMA.cv.enabled     = _CreateConVar("ttt_karma", "1", FCVAR_ARCHIVE)
KARMA.cv.strict      = _CreateConVar("ttt_karma_strict", "1")
KARMA.cv.starting    = _CreateConVar("ttt_karma_starting", "1000")
KARMA.cv.max         = _CreateConVar("ttt_karma_max", "1000")
KARMA.cv.ratio       = _CreateConVar("ttt_karma_ratio", "0.001")
KARMA.cv.killpenalty = _CreateConVar("ttt_karma_kill_penalty", "15")
KARMA.cv.roundheal   = _CreateConVar("ttt_karma_round_increment", "5")
KARMA.cv.clean  = _CreateConVar("ttt_karma_clean_bonus", "30")
KARMA.cv.tbonus = _CreateConVar("ttt_karma_traitorkill_bonus", "40")
KARMA.cv.tratio = _CreateConVar("ttt_karma_traitordmg_ratio", "0.0003")
KARMA.cv.debug  = _CreateConVar("ttt_karma_debugspam", "0")

KARMA.cv.persist = _CreateConVar("ttt_karma_persist", "0")
KARMA.cv.falloff = _CreateConVar("ttt_karma_clean_half", "0.25")

KARMA.cv.autokick  = _CreateConVar("ttt_karma_low_autokick", "1")
KARMA.cv.kicklevel = _CreateConVar("ttt_karma_low_amount", "450")
KARMA.cv.autoban   = _CreateConVar("ttt_karma_low_ban", "1")
KARMA.cv.bantime   = _CreateConVar("ttt_karma_low_ban_minutes", "60")
...
function KARMA.GetHurtReward(dmg)
   return config.max:GetFloat() * _math_Clamp(dmg * config.tratio:GetFloat(), 0, 1)
end
...
function KARMA.GivePenalty(ply, penalty, victim)
        if not _hook_Call( "TTTKarmaGivePenalty", nil, ply, penalty, victim ) then
                ply:SetLiveKarma(_math_max(ply:GetLiveKarma() - penalty, 0))
        end
end

function KARMA.GiveReward(ply, reward)
   reward = KARMA.DecayedMultiplier(ply) * reward
   ply:SetLiveKarma(_math_min(ply:GetLiveKarma() + reward, config.max:GetFloat()))
   return reward
end
...
function KARMA.ApplyKarma(ply)
   ...
   ply:SetDamageFactor(_math_Clamp(df, 0.1, 1.0))

   if IsDebug() then
      _print(_Format("%s has karma %f and gets df %f", ply:Nick(), ply:GetBaseKarma(), df))
   end
end
...
local function WasAvoidable(attacker, victim, dmginfo)
   ...
   if attacker:IsTraitor() and victim:IsTraitor() and _IsValid(infl) and infl.Avoidable then
      return true
   end
...
function KARMA.Hurt(attacker, victim, dmginfo)
   if not _IsValid(attacker) or not _IsValid(victim) then return end
   if attacker == victim then return end
   if not attacker:IsPlayer() or not victim:IsPlayer() then return end

   -- Ignore excess damage
   local hurt_amount = _math_min(victim:Health(), dmginfo:GetDamage())
   ...
   if attacker:GetTraitor() == victim:GetTraitor() then
      ...
      if IsDebug() then
         _print(_Format("%s (%f) attacked %s (%f) for %d and got penalised for %f", attacker:Nick(), attacker:GetLiveKarma(), victim:Nick(), victim:GetLiveKarma(), hurt_amount, penalty))
      end
   elseif (not attacker:GetTraitor()) and victim:GetTraitor() then
      ...
      if IsDebug() then
         _print(_Format("%s (%f) attacked %s (%f) for %d and got REWARDED %f", attacker:Nick(), attacker:GetLiveKarma(), victim:Nick(), victim:GetLiveKarma(), hurt_amount, reward))
      end
   end
end
...
function KARMA.Killed(attacker, victim, dmginfo)
   ...
   if attacker:GetTraitor() == victim:GetTraitor() then
      ...
      if IsDebug() then
         _print(_Format("%s (%f) killed %s (%f) and gets penalised for %f", attacker:Nick(), attacker:GetLiveKarma(), victim:Nick(), victim:GetLiveKarma(), penalty))
      end
   elseif (not attacker:GetTraitor()) and victim:GetTraitor() then
      ...
      if IsDebug() then
         _print(_Format("%s (%f) killed %s (%f) and gets REWARDED %f", attacker:Nick(), attacker:GetLiveKarma(), victim:Nick(), victim:GetLiveKarma(), reward))
      end
   end
end
...
function KARMA.DecayedMultiplier(ply)
   ...
   elseif k < max then
      ...
      local half     = _math_Clamp(config.falloff:GetFloat(), 0.01, 0.99)
...
function KARMA.RoundIncrement()
   ...
   for _, ply in _ipairs(_player_GetAll()) do
      if ply:IsDeadTerror() and ply.death_type ~= KILL_SUICIDE or not ply:IsSpec() then
         ...
         if IsDebug() then
            _print(ply, "gets roundincr", incr)
         end
      end
   end
...
function KARMA.Rebase()
   for _, ply in _ipairs(_player_GetAll()) do
      if IsDebug() then
         _print(ply, "rebased from", ply:GetBaseKarma(), "to", ply:GetLiveKarma())
      end
...
function KARMA.InitPlayer(ply)
   ...
   k = _math_Clamp(k, 0, config.max:GetFloat())
...
function KARMA.Recall(ply)
   if config.persist:GetBool()then
      ...
      if ply:IsFullyAuthenticated() then
         local k = _tonumber(ply:GetPData("karma_stored", nil))
         if k then
            return k
         end
      end
...
function KARMA.LateRecallAndSet(ply)
   local k = _tonumber(ply:GetPData("karma_stored", KARMA.RememberedPlayers[ply:SteamID()]))
...
function KARMA.CheckAutoKick(ply)
   if ply:GetBaseKarma() <= config.kicklevel:GetInt() then
      if _hook_Call("TTTKarmaLow", GAMEMODE, ply) == false then
         return
      end
      _ServerLog(ply:Nick() .. " autokicked/banned for low karma.\n")
      ...
      if config.persist:GetBool() then
         local k = _math_Clamp(config.starting:GetFloat() * 0.8, config.kicklevel:GetFloat() * 1.1, config.max:GetFloat())
...
function KARMA.PrintAll(printfn)
   for _, ply in _ipairs(_player_GetAll()) do
      printfn(_Format("%s : Live = %f -- Base = %f -- Dmg = %f\n",
                     ply:Nick(),
