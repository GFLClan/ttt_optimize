...
local _table_Merge = table.Merge
...
function GetEquipmentForRole(role)
   ...
   if not Equipment then
      ...
      local tbl = _table_Copy(EquipmentItems)

      -- find buyable weapons to load info from
      for k, v in _pairs(weapons.GetList()) do
         if v and v.CanBuy then
            ...
            _table_Merge(base, data)

            -- add this buyable weapon to all relevant equipment tables
            for _, r in _pairs(v.CanBuy) do
               _table_insert(tbl[r], base)
            end
         end
      end

      -- mark custom items
      for r, is in _pairs(tbl) do
         for _, i in _pairs(is) do
            if i and i.id then
               i.custom = not _table_HasValue(DefaultEquipment[r], i.id)
            end
         end
      end
...
local function ItemIsWeapon(item) return not _tonumber(item.id) end
local function CanCarryWeapon(item) return _LocalPlayer():CanCarryType(item.kind) end

local color_bad = _Color(220, 60, 60, 255)
local color_good = _Color(0, 200, 0, 255)

-- Creates tabel of labels showing the status of ordering prerequisites
local function PreqLabels(parent, x, y)
   local tbl = {}

   tbl.credits = _vgui_Create("DLabel", parent)
   tbl.credits:SetTooltip(GetTranslation("equip_help_cost"))
   tbl.credits:SetPos(x, y)
   tbl.credits.Check = function(s, sel)
                          local credits = _LocalPlayer():GetCredits()
                          return credits > 0, GetPTranslation("equip_cost", {num = credits})
                       end

   tbl.owned = _vgui_Create("DLabel", parent)
   ...
   tbl.owned.Check = function(s, sel)
                        ...
                        elseif (not ItemIsWeapon(sel)) and _LocalPlayer():HasEquipmentItem(sel.id) then
   ...
   tbl.bought = _vgui_Create("DLabel", parent)
   ...
   tbl.bought.Check = function(s, sel)
                         if sel.limited and _LocalPlayer():HasBought(_tostring(sel.id)) then
...
local function DrawSelectedEquipment(pnl)
   ...
   _surface_DrawOutlinedRect(0, 0, pnl:GetWide(), pnl:GetTall())
end
...
_vgui_Register("EquipSelect", PANEL, "DPanelSelect")


local SafeTranslate = LANG.TryTranslation

local color_darkened = _Color(255,255,255, 80)
...
local function TraitorMenuPopup()
   ...
   if eqframe and _IsValid(eqframe) then eqframe:Close() end

   local credits = ply:GetCredits()
   local can_order = credits > 0

   local dframe = _vgui_Create("DFrame")
   local w, h = 570, 412
   dframe:SetSize(w, h)
   dframe:Center()
   ...
   local dsheet = _vgui_Create("DPropertySheet", dframe)
   ...
   local dequip = _vgui_Create("DPanel", dsheet)
   ...
   local owned_ids = {}
   for _, wep in _ipairs(ply:GetWeapons()) do
      if _IsValid(wep) and wep:IsEquipment() then
         _table_insert(owned_ids, wep:GetClass())
      end
   end
   ...
   local dlist = _vgui_Create("EquipSelect", dequip)
   dlist:SetPos(0,0)
   ...
   for k, item in _pairs(items) do
      ...
      if item.material then
         if item.custom then
            -- Custom marker icon
            ic = _vgui_Create("LayeredIcon", dlist)

            local marker = _vgui_Create("DImage")
         ...
         elseif not ItemIsWeapon(item) then
            ic = _vgui_Create("SimpleIcon", dlist)
         else
            ic = _vgui_Create("LayeredIcon", dlist)
         end
         ...
         if ItemIsWeapon(item) then
            local slot = _vgui_Create("SimpleIconLabelled")
      ...
      elseif item.model then
         ic = _vgui_Create("SpawnIcon", dlist)
         ic:SetModel(item.model)
      else
         _ErrorNoHalt("Equipment item does not have model or material specified: " .. _tostring(item) .. "\n")
      end
      ...
      if ((not can_order) or
          -- already owned
          _table_HasValue(owned_ids, item.id) or
          (_tonumber(item.id) and ply:HasEquipmentItem(_tonumber(item.id))) or
          -- already carrying a weapon for this slot
          (ItemIsWeapon(item) and (not CanCarryWeapon(item))) or
          -- already bought the item before
          (item.limited and ply:HasBought(_tostring(item.id)))) then
   ...
   local dinfobg = _vgui_Create("DPanel", dequip)
   dinfobg:SetPaintBackground(false)
   dinfobg:SetSize(diw, dih)
   dinfobg:SetPos(dlistw + m, 0)

   local dinfo = _vgui_Create("ColoredBox", dinfobg)
   dinfo:SetColor(_Color(90, 90, 95))
   dinfo:SetPos(0,0)
   dinfo:StretchToParent(0, 0, 0, dih - 135)

   local dfields = {}
   for _, k in _ipairs(fieldstbl) do
      dfields[k] = _vgui_Create("DLabel", dinfo)
   ...
   local dhelp = _vgui_Create("ColoredBox", dinfobg)
   dhelp:SetColor(_Color(90, 90, 95))
   ...
   local dconfirm = _vgui_Create("DButton", dinfobg)
   ...
   -- Weapon/item control
   if _IsValid(ply.radio) or ply:HasWeapon("weapon_ttt_radio") then
   ...
   dconfirm.DoClick = function()
                         ...
                         _RunConsoleCommand("ttt_order_equipment", choice.id)
                         dframe:Close()
                      end
   ...
   dsheet.OnTabChanged = function(s, old, new)
                            if not _IsValid(new) then return end
   ...
   local dcancel = _vgui_Create("DButton", dframe)
...
_concommand_Add("ttt_cl_traitorpopup", TraitorMenuPopup)

local function ForceCloseTraitorMenu(ply, cmd, args)
   if _IsValid(eqframe) then
      eqframe:Close()
   end
end
_concommand_Add("ttt_cl_traitorpopup_close", ForceCloseTraitorMenu)

function GM:OnContextMenuOpen()
   local r = GetRoundState()
   if r == ROUND_ACTIVE and not (_LocalPlayer():GetTraitor() or _LocalPlayer():GetDetective()) then
      return
   ...
   if _IsValid(eqframe) then
      ...
      _RunConsoleCommand("ttt_cl_traitorpopup")
   end
end
...
local function ReceiveEquipment()
   ...
   ply.equipment_items = _net_ReadUInt(16)
end
_net_Receive("TTT_Equipment", ReceiveEquipment)

local function ReceiveCredits()
   local ply = _LocalPlayer()
   if not _IsValid(ply) then return end

   ply.equipment_credits = _net_ReadUInt(8)
end
_net_Receive("TTT_Credits", ReceiveCredits)
...
local function ReceiveBought()
   ...
   ply.bought = {}
   local num = _net_ReadUInt(8)
   for i=1,num do
      local s = _net_ReadString()
      if s != "" then
         _table_insert(ply.bought, s)
      end
   end
   ...
   if num != #ply.bought and r < 10 then -- r is an infinite loop guard
      _RunConsoleCommand("ttt_resend_bought")
      r = r + 1
   else
      r = 0
   end
end
_net_Receive("TTT_Bought", ReceiveBought)

-- Player received the item he has just bought, so run clientside init
local function ReceiveBoughtItem()
   local is_item = _net_ReadBit() == 1
   local id = is_item and _net_ReadUInt(16) or _net_ReadString()

   -- I can imagine custom equipment wanting this, so making a hook
   _hook_Run("TTTBoughtItem", is_item, id)
end
_net_Receive("TTT_BoughtItem", ReceiveBoughtItem)
