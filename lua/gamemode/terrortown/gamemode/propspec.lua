...
local propspec_toggle = _CreateConVar("ttt_spec_prop_control", "1")

local propspec_base = _CreateConVar("ttt_spec_prop_base", "8")
local propspec_min = _CreateConVar("ttt_spec_prop_maxpenalty", "-6")
local propspec_max = _CreateConVar("ttt_spec_prop_maxbonus", "16")

function PROPSPEC.Start(ply, ent)
   ply:Spectate(OBS_MODE_CHASE)
   ply:SpectateEntity(ent, true)

   local bonus = _math_Clamp(_math_ceil(ply:Frags() / 2), propspec_min:GetInt(), propspec_max:GetInt())
...
local function IsWhitelistedClass(cls)
   return (_string_match(cls, "prop_physics*") or
           _string_match(cls, "func_physbox*"))
end

function PROPSPEC.Target(ply, ent)
   if not propspec_toggle:GetBool() then return end
   if (not _IsValid(ply)) or (not ply:IsSpec()) or (not _IsValid(ent)) then return end

   if _IsValid(ent:GetNWEntity("spec_owner", nil)) then return end

   local phys = ent:GetPhysicsObject()

   if ent:GetName() != "" and (not GAMEMODE.propspec_allow_named) then return end
   if (not _IsValid(phys)) or (not phys:IsMoveable()) then return end
...
function PROPSPEC.End(ply)
   ...
   _timer_Simple(0.1, function()
                        if _IsValid(ply) then ply:ResetViewRoll() end
                     end)
end

local propspec_force = _CreateConVar("ttt_spec_prop_force", "110")

function PROPSPEC.Key(ply, key)
   local ent = ply.propspec.ent
   local phys = _IsValid(ent) and ent:GetPhysicsObject()
   if (not _IsValid(ent)) or (not _IsValid(phys)) then 
      PROPSPEC.End(ply)
      return false
   end
   ...
   if pr.t > _CurTime() then return true end

   if pr.punches < 1 then return true end

   local m = _math_min(150, phys:GetMass())
   ...
   if key == IN_JUMP then
      -- upwards bump
      phys:ApplyForceCenter(_Vector(0,0, mf))
      pr.t = _CurTime() + 0.05
   ...
   elseif key == IN_MOVELEFT then
      phys:AddAngleVelocity(_Vector(0, 0, 200))
      phys:ApplyForceCenter(_Vector(0,0, mf / 3))
   ...
   pr.punches = _math_max(pr.punches - 1, 0)
   ply:SetNWFloat("specpunches", pr.punches / pr.max)

   return true
end

local propspec_retime = _CreateConVar("ttt_spec_prop_rechargetime", "1")
function PROPSPEC.Recharge(ply)
   local pr = ply.propspec
   if pr.retime < _CurTime() then
      pr.punches = _math_min(pr.punches + 1, pr.max)
      ply:SetNWFloat("specpunches", pr.punches / pr.max)

      pr.retime = _CurTime() + propspec_retime:GetFloat()
   end
end
